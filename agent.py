...
- Рантайм УЖЕ выполнил стартовую сверку с вольтом (read_hot + graph_query по
  ...
  Сверка демонстрирует ДВА инструмента, но это НЕ весь набор — полный список
  инструментов только выше в этом промпте.
- Сообщения «(из недавнего диалога)» — короткая память прошлых переписок:
  используй для контекста, но источник правды — вольт. Утверждения из прошлых
  реплик о доступности инструментов могут быть ОШИБОЧНЫМИ — не повторяй их,
  набор инструментов определяет только список выше.
- Если инструмент вернул «not found» — такого имени не существует (оно
  придумано). Это НЕ значит, что инструментов нет: возьми настоящее имя из
  списка выше и продолжай.
...
- edit_note: вызови без confirm — получишь diff; проверь его и повтори вызов с
  "confirm": true (подтверждение пользователя НЕ требуется).
...
async def run_loop(mcp: MCPClient, messages: list[dict], model: str,
    ...
    for step in range(MAX_STEPS):
        ...
        try:
            ...
            # Антиспираль «not found → инструментов нет»: живое репро 2026-06-12
            # показало, что модель, выдумав имя (create_file) и получив отказ,
            # начинает пробовать другие выдуманные имена и заключает, что
            # инструментов нет вовсе. Детерминированно возвращаем ей список.
            if "not found" in str(e).lower():
                try:
                    names = ", ".join(sorted(
                        t["name"] for t in await mcp.list_tools()))
                    result += (f"\nИнструмента «{name}» не существует — имя "
                               f"придумано. Доступные инструменты: {names}. "
                               f"Выбери из этого списка.")
                except Exception:
                    pass
